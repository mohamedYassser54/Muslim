...
import dataquran  from '../../data/DQAnNisa'
  ...
  useEffect(() => {
    if (swiper) {
      ...
      swiper.on('slideChange', () => {
        window.localStorage.setItem('swiperSlideIndex4', swiper.activeIndex);
      });

      // إعادة تعيين حركة الصفحة إلى الشريحة المحفوظة عند تحميل المكون
      const savedSlideIndex = window.localStorage.getItem('swiperSlideIndex4');
      if (savedSlideIndex) {
