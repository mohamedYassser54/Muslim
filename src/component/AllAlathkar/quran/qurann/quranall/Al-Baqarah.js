...
import dataquran  from '../../data/DQAlBaqarah'
  ...
  useEffect(() => {
    if (swiper) {
      ...
      swiper.on('slideChange', () => {
        window.localStorage.setItem('swiperSlideIndex2', swiper.activeIndex);
      });

      // إعادة تعيين حركة الصفحة إلى الشريحة المحفوظة عند تحميل المكون
      const savedSlideIndex = window.localStorage.getItem('swiperSlideIndex2');
      if (savedSlideIndex) {
  ...
  return (
    <div>
      <div className='btnqu'>
        ...
        <NavLink className="NavLink" to="/Imraan">
